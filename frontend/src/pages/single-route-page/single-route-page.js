...
import {
    exitRoutePage, getFilteredRoutes, getGradingSystemMapping,
    GlobalState, gradeToDisplayName,
    isAdmin,
    onBackClicked,
    toggleLikeRoute,
...
<!--            <div id="grade-prefix">Grade:</div>-->
            <x-button id="grade-button"
                ...
                ${() => gradeToDisplayName(props.route?.grade)}
                <x-icon icon="arrow_drop_down"></x-icon>
            </x-button>
        </div>
        <x-dialog id="grade-dialog"
                  class="header-dialog">
            ${() => Object.entries(getGradingSystemMapping())
            .map(([gradeName, grade]) => html()`
                <div class="item"
