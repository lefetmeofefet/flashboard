...
import {
    ...
    GlobalState, gradeToDisplayName, seedShuffleSort, sortRoutes,
} from "../../state.js";
...
createYoffeeElement("routes-filter", (props, self) => {
    ...
    function createFilter(filterType) {
        ...
        if (filterType === FILTER_TYPES.GRADE) {
            setFilter(FILTER_TYPES.GRADE, {min: 0, max: Object.keys(getGradingSystemMapping()).length - 1})
    ...
    // Open the grade filter by default
    self.onConnect = () => {
        if (_connectedOnce) {
            return
        }
    ...
    let gradeIndexToDisplayName = index => {
        let gradeName = Object.keys(getGradingSystemMapping())[index]
        let grade = getGradingSystemMapping()[gradeName]
        return gradeToDisplayName(grade)
    }
...
<x-button class="tag"
    ...
    ${() => {
        // Renders filter
        if (filter.type === FILTER_TYPES.GRADE) {
            return filter.value.min === filter.value.max ? `${gradeIndexToDisplayName(filter.value.min)}` : `${gradeIndexToDisplayName(filter.value.min)} - ${gradeIndexToDisplayName(filter.value.max)}`
        } else if (filter.type === FILTER_TYPES.SETTER) {
            return html()`
    ...
    ${() => {
        ...
        if (state.editedFilterType === FILTER_TYPES.GRADE) {
            ...
            <div id="v-slider-container">
                <div class="v-number">${() => gradeIndexToDisplayName(sliderState.min)}</div>
                <x-double-slider initvaluemin=${existingFilter.value.min}
                                 initvaluemax=${existingFilter.value.max}
                                 min=0
                                 max=${() => Object.keys(getGradingSystemMapping()).length - 1}
                                 step
                                 updated=${() => ({min, max}) => {
                ...
                <div class="v-number">${() => gradeIndexToDisplayName(sliderState.max)}</div>
            </div>
            `
